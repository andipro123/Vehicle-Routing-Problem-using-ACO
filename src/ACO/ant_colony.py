import sys
sys.path.append('../')
from tools.parse import getData
...
class Solver:
    ...
    def get_route_cost_with_depo(self, route, graph):
        depot_costs = round(graph.adjacency_map[1][route[0]], 5) + round(
            graph.adjacency_map[route[-1]][1], 5)
    ...
    def apply_two_opt(self, initial_solution, graph):
        best_routes = [[1] + self.get_optimal_route(route[1:-1], graph) + [1]
                       for route in initial_solution[0]]
        return best_routes, sum(
            [self.get_route_cost(route, graph) for route in best_routes])

    def __call__(self, cfg, verbose=True):
        capacity, graph_data, demand, optimal_value, name, n_trucks = getData(
            cfg.FILE_NAME)
        ...
        for i in range(1, cfg.NUM_ITERATIONS + 1):
            ...
            candidate_best_solution = min(solutions,
                                          key=lambda solution: solution[1])
            if cfg.USE_2_OPT_STRATEGY:
                candidate_best_solution = self.apply_two_opt(
                    candidate_best_solution, graph)
            graph.update_pheromone_map(solutions)
            graph.global_update_pheromone_map(candidate_best_solution,
                                              capacity)
            if not best_solution or candidate_best_solution[1] < best_solution[
                    1]:
            ...
            if verbose and i % 100 == 0:
                print("Best solution in iteration {}/{} = {}".format(
                    i, cfg.NUM_ITERATIONS, best_solution[1]))
