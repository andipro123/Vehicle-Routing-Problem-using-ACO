...
class Ant:
    ...
    def select_next_city(self, current_city):
        available_cities = [
            city for city in self.cities_left
            if self.capacity >= self.graph.demand_map[city]
        ]
        ...
        scores = [
            pow(self.graph.pheromone_map[current_city][city], self.cfg.ALPHA) *
            pow(1 / (self.graph.adjacency_map[current_city][city] + 1e-10),
                self.cfg.BETA) for city in available_cities
        ]
    ...
    def move_to_city(self, current_city, next_city):
        ...
        self.total_path_cost += self.graph.adjacency_map[current_city][
            next_city]
